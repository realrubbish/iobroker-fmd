...
### Requirement: Status panel shows live connection state
...
The Status panel SHALL contain a `Test Connection` button. Clicking the button SHALL send a `testConnection` message to the adapter instance via the standard ioBroker `sendTo` admin mechanism and SHALL show the result inline (success / failure with message).

#### Scenario: Successful test
- **WHEN** the user clicks `Test Connection`
- **AND** the adapter can reach the FMD server with the configured credentials
- **THEN** the panel shows a success message (e.g. "Connection OK")

#### Scenario: Failed test
- **WHEN** the user clicks `Test Connection`
- **AND** the adapter cannot reach the FMD server
- **THEN** the panel shows an error message containing the failure reason
