...
- [x] 5.2 `npm run ring:smoke:verify`. Expect
  ...
  > succeeded.
- [ ] 5.3 (Optional but recommended) `npm run
  ...
  > Deferred to the user; needs real
  > `FMD_SERVER_URL`/`FMD_USERNAME`/`FMD_PASSWORD`/
  > `FMD_DEVICE_ID` from the dev environment.
...
- [ ] 6.3 `docker exec iobroker-fmd-dev iobroker
  ...
  true`.
- [ ] 6.4 The phone rings within ~5 s of the
