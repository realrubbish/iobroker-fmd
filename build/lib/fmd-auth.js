...
class FmdAuth {
     ...
     */
    async getPrivateKey(accessToken, password) {
        try {
     ...
     */
    async decryptPrivateKey(wrappedB64, password) {
        ...
        if (concatBytes.length < minLen) {
            throw new Error(`Wrapped private key too short: ${concatBytes.length} bytes (need at least ${minLen})`);
        }
        ...
        const aesKey = (await (0, hash_wasm_1.argon2id)({
        ...
        try {
            const decipher = (0, crypto_1.createDecipheriv)("aes-256-gcm", Buffer.from(aesKey), Buffer.from(ivBytes));
            ...
            const errorMsg = err instanceof Error ? err.message : String(err);
            throw new Error(`AES-GCM decrypt failed (likely wrong password or corrupted wrapped key): ${errorMsg}`);
        }
    }
    /**
