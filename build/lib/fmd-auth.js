...
class FmdAuth {
    ...
    async getSalt() {
        try {
            const response = await this.httpClient.get("/api/v1/auth/salt");
    ...
    async deriveKey(salt) {
        try {
            ...
            const keyMaterial = await crypto.subtle.importKey("raw", encoder.encode(this.config.password), "PBKDF2", false, ["deriveBits"]);
            const derivedBits = await crypto.subtle.deriveBits({
                name: "PBKDF2",
                salt: saltBytes,
                iterations: 3,
                hash: "SHA-256",
            }, keyMaterial, 256 // 32 bytes * 8 bits
            );
    ...
    async getPrivateKey(accessToken) {
        try {
            const response = await this.httpClient.get("/api/v1/auth/key", {
                headers: {
                    IDT: accessToken,
                },
            });
